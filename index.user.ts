...
interface CalcType {
  ...
  expressionAnalysis: { [key: string]: ExpressionAnalysis }
...
(() => {
  ...
  let globalVariablesObject: { [key: string]: string } = {};
  let id = 1;
  let shadeId = 1;
  ...
  class LinkedExpression extends LinkedVariable {
    variables: {[key: string]: LinkedVariable}

    constructor(reference: string, _variables: {[key: string]: LinkedVariable}) {
  ...
  const shadingData: { [key: string]: { x: LinkedVariable, y: LinkedVariable } } = {
    lastUpperBoundary: {
  ...
  function convertFromStandard(latex: string, _id: string) {
    ...
    if (conicType === -1) {
      ...
      if (match) {
        ...
        if (domains) {
          let { xMin, xMax, yMin, yMax } = parseDomains(domains);
            ...
            if (i === 0) {
              if (conicType !== 6) {
                newExpressionLatex += generateBounds(
                  new LinkedVariable(`x_{${conicId}ca}`, xMin),
                  new LinkedVariable(`y_{${conicId}ca}`, yMin),
                  new LinkedVariable(`x_{${conicId}cb}`, xMax),
                  new LinkedVariable(`y_{${conicId}cb}`, yMax),
                ).reference;
              }
            }
