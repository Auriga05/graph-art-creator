...
export class HorizontalParabola extends Graph implements Initializable, Conic {
  ...
  static xExpression = ['h_{1}+\\frac{\\left(y-k_{1}\\right)^{2}}{4c_{1}}']
  static yExpression = ['k_{1}-\\sqrt{4c_{1}\\left(x-h_{1}\\right)}', 'k_{1}+\\sqrt{4c_{1}\\left(x-h_{1}\\right)}']
  static graphTypeName = "horizontal_parabola"
  static expressionFormat = [ // Horizontal Parabola (x)
    ...
    ...HorizontalParabola.yExpression.map((yExpression, c) => ({ latex: `f_{1y${String.fromCharCode(97 + c)}}(x)=${yExpression}`, types: ['y_expression'], name: `f_{1y${String.fromCharCode(97 + c)}}` })),
    ...HorizontalParabola.xExpression.map((xExpression, c) => ({ latex: `f_{1x${String.fromCharCode(97 + c)}}(y)=${xExpression}`, types: ['x_expression'], name: `f_{1x${String.fromCharCode(97 + c)}}` })),
  ]
