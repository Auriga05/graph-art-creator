...
import { LinkedVariable, getVariable, substitute, getDomains, setVariable, getValue } from "../lib/lib";
...
export class VerticalHyperbola extends Graph implements Initializable, Conic {
  ...
  static xExpression = ['h_{1}-\\frac{b_{1}}{a_{1}}\\sqrt{\\left(y-k_{1}\\right)^{2}-a_{1}^{2}}', 'h_{1}+\\frac{b_{1}}{a_{1}}\\sqrt{\\left(y-k_{1}\\right)^{2}-a_{1}^{2}}']
  static yExpression = ['k_{1}-\\frac{a_{1}}{b_{1}}\\sqrt{b_{1}^{2}+\\left(x-h_{1}\\right)^{2}}', 'k_{1}+\\frac{a_{1}}{b_{1}}\\sqrt{b_{1}^{2}+\\left(x-h_{1}\\right)^{2}}']
  static graphTypeName = "vertical_hyperbola"
  static expressionFormat = [ // Vertical Hyperbola (y)
    ...
    ...VerticalHyperbola.yExpression.map((yExpression, c) => ({ latex: `f_{1y${String.fromCharCode(97 + c)}}(x)=${yExpression}`, types: ['y_expression'], name: `f_{1y${String.fromCharCode(97 + c)}}` })),
    ...VerticalHyperbola.xExpression.map((xExpression, c) => ({ latex: `f_{1x${String.fromCharCode(97 + c)}}(y)=${xExpression}`, types: ['x_expression'], name: `f_{1x${String.fromCharCode(97 + c)}}` })),
  ]
  ...
  getRelevant(axis: string) {
    ...
    if (axis === 'x') {
      if (getValue(yMax) < h) relevantIndices.push(0);
      if (getValue(yMin) > h) relevantIndices.push(1);
    }
    if (axis === 'y') {
      if (getValue(yMax) < k + a) relevantIndices.push(0);
      if (getValue(yMin) > k - a) relevantIndices.push(1);
    }  
